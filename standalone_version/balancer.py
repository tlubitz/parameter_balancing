...
class ParameterBalancing:
    ...
    def get_parameter_information(self, alternate_prior = None):
        ...
        for row in sbtab_prior.value_rows:
            if row[sbtab_prior.columns_dict['!QuantityType']] not in name2index.keys():
                continue
            if row[sbtab_prior.columns_dict['!UseAsPriorInformation']] == '0':
                continue
            if len(row) == len(sbtab_prior.columns):
                self.quantity2identifier[row[sbtab_prior.columns_dict['!QuantityType']]] = \
                                 row[sbtab_prior.columns_dict['!Abbreviation']]
                self.quantity_type2unit[row[sbtab_prior.columns_dict['!QuantityType']]] = \
                                         row[sbtab_prior.columns_dict['!Unit']]
                ...
                self.matrix_info[row[sbtab_prior.columns_dict['!QuantityType']]] = \
    ...
    def tidy_up_sbtab(self, ig_bounds):
        ...
        functionality or delete entries that must not be used in parameter
        balancing
        '''
        consistent_rows = []
        log_header = False
            ...
            if len(row) == len(self.sbtab.value_rows[0]):
                ...
                if 'boundary_values' in self.parameter_dict and ig_bounds:
                    if self.parameter_dict['boundary_values'] == 'ignore':
                        if row[self.sbtab.columns_dict['!QuantityType']] != '':
                            if self.pmin[row[self.sbtab.columns_dict['!QuantityType']]] is not None:
                                if float(row[mean_column]) < \
                                   float(self.pmin[row[self.sbtab.columns_dict['!QuantityType']]]):
                                    ...
                                    continue
                        if row[self.sbtab.columns_dict['!QuantityType']] != '':
                            if self.pmax[row[self.sbtab.columns_dict['!QuantityType']]] is not None:
                                if float(row[mean_column]) > \
                                   float(self.pmax[row[self.sbtab.columns_dict['!QuantityType']]]):
                                    if log_header is False:
                                        self.log += '\n### Warnings about ignored values that '\
                                                    'lie out of the boundaries ### \n'
                                        log_header = True
    ...
    def mean_row(self, name, quantity):
        ...
        if quantity in self.quantity_type2median_std:
            ...
            std = scipy.mean(stds)
            if quantity not in self.thermodynamics:
                median = numpy.exp(self.normal_to_log([mean],
                                                      [std],
                                                      False)[0])[0]
            else: median = mean
        ...
        if pseudos:
            ...
            for i, row in enumerate(sbtab.value_rows):
                if row[sbtab.columns_dict['!QuantityType']] in self.pseudo_list:
                    try:
                        row[sbtab.columns_dict['!Mode']] = \
                                                        str(pseudos[row[0]][0])
                        row[sbtab.columns_dict['!UnconstrainedGeometricMean']] = \
                                                        str(pseudos[row[0]][0])
                        row[sbtab.columns_dict['!UnconstrainedGeometricStd']] = \
                                                        str(pseudos[row[0]][1])
            ...
            (self.log_means,
             self.log_stds) = self.med10_std_to_log(means,
                                                    stds,
                                                    self.pseudo_list)
    ...
    def make_default_table(self):
        '''
        generate table of values for every parameter for every reaction/species
        '''
        medians = []
        stds = []
        quantities = []
        ...
        (self.log_means, self.log_stds) = self.med10_std_to_log(medians,
                                                                stds,
                                                                quantities)

        self.prior_values = {}
        for i, quantity in enumerate(quantities):
            self.prior_values[quantity] = [(self.log_means[i],
                                            self.log_stds[i])]
    ...
    def log_to_normal(self, log_means, log_stds, types=None):
        ...
        for i, log_mean in enumerate(log_means):
            if types and types[i] in self.thermodynamics:
                ...
                means.append(numpy.exp(log_mean + 0.5 * \
                                       numpy.square(log_stds[i])))
    ...
    def med10_std_to_log(self, medians, stdlogs, types):
        '''
        this is the users default choice for missing values: inserted are the
        median and the stdlog10, the output are the corresponding mean value
    ...
    def add_config_to_log(self):
        ...
        '''
        self.log += '\n### You have used an options file for the parameter ba'\
...
###############################################################################
    ...
    def print_warning(self, type, flag):
        if self.warned:
            print('There was an error in the numerics. This may be due to bro'\
                  'adly chosen probability distributions. Try the usage of ps'\
                  'eudo values for a fix.')
            self.warned = False
    ...
    def make_balancing(self, sbtab, sbtab_old, pmin, pmax, parameter_dict):
        ...
        # KEY PART: calculating the posteriori covariance matrix C_post and the
        #           posteriori mean vector mean_post
        self.calculate_posteriori()

        # make normal values again
        (self.mean_post,
         self.stds_post) = self.log_to_normal(self.x_post,
                                              self.stds_log_post,
                                              self.quantities)
        ...
        if self.bounds_inc.count((None, None)) != len(self.bounds_inc):  # and False:
            # generating the medians to bound them
            print('in optimiser')
            medians = []
            medstds = []

            (self.means_inc,
             self.stds_inc) = self.log_to_normal(self.means_post_inc,
                                                 self.stds_log_inc,
                                                 self.quantities_inc)

            for i, value in enumerate(self.means_inc):
                if self.quantities_inc[i] not in self.thermodynamics:
                    (log_mean,
                     log_std) = self.normal_to_log([self.means_inc[i]],
                                                   [self.stds_inc[i]],
            ...
            def log_mean_post_func(q):
                return numpy.dot(((q - medians).transpose()),
                                 (numpy.dot(numpy.linalg.inv(self.C_post),
                                            (q - medians))))

            # setting proper boundaries for parameters that have no boundaries
            # set in SBtab
            new_boundaries = []
            is_logarithmic = []
                    ...
                    if name2index[self.quantities_inc[i]] in thermodynamic_indices:
                        ...
                        if (max(medians[i] - medstds[i] * 2, -3000)) < \
                           (min(3000, medians[i] + medstds[i] * 2)):
                            ...
                            new_boundaries.append((medians[i] + medstds[i] * 2,
                                                   medians[i] - medstds[i] * 2))
                    # setting boundaries for all other parameters
                    else:
                        is_logarithmic.append(True)
                        if (medians[i] - medstds[i] * 4) < \
                           (medians[i] + medstds[i] * 4):
                            new_boundaries.append((max(medians[i] - medstds[i] * 4,
                                                       0.00001),
                                                   medians[i] + medstds[i] * 4))
                        else:
                            new_boundaries.append((medians[i] + medstds[i] * 4,
                                                   medians[i] - medstds[i] * 4))
                else:
                    is_logarithmic.append(True)
            ...
            new_medians = misc.fmin_gen(log_mean_post_func,
                                        numpy.array(medians),
                                        population_size=20,
                                        survivors=5, generations=500,
                                        bounds=proper_boundaries, use_pp=False,
                                        variable_is_logarithmic=is_logarithmic,
                                        disp=1)
            for i, value in enumerate(new_medians):
                if value + 0.001 < proper_boundaries[i][0] or \
                   value - 0.001 > proper_boundaries[i][1]:
                    self.quantities_inc[i]
                    print('NEW_MODE value out of bound: ',
                          value,
                          ' [bounds: ', proper_boundaries[i], ']')

            (new_medians_log,
             new_stds_log) = self.normal_to_log(new_medians,
                                                self.stds_inc,
                                                self.quantities_inc)
            self.C_xpost = numpy.dot((numpy.dot(self.Q, self.C_post)),
                                     self.Q.transpose())
            self.x_post = numpy.dot(self.Q, new_medians_log)
            self.stds_log_post = self.extract_cpost()

            (self.mean_post_opt,
             self.stds_post_opt) = self.log_to_normal(self.x_post,
                                                      self.stds_log_post,
                                                      self.quantities)
            self.optimized = True

        #################################################################
        # make value-dictionaries to realize the insertion of the computed
        # values into the SBtab-file in GUI
    ...
    def get_sheet(self):
        ...
        '''
        sheet = {"equilibrium constant": ((-1 / 2.4790, "A"),
                                          0, 0, 0, 0, 0, 0),
                 "substrate catalytic rate constant": ((-0.5 / 2.4790, "A"), 1,
                                                       (-0.5, "Z"), 0, 0, 0, 0),
                 "product catalytic rate constant": ((0.5 / 2.4790, "A"), 1,
                                                     (0.5, "Z"), 0, 0, 0, 0),
                 "forward maximal velocity": ((-0.5 / 2.4790, "A"), 1,
                                              (-0.5, "Z"), 0, 0, 1, 0),
                 "reverse maximal velocity": ((0.5 / 2.4790, "A"), 1,
                                              (0.5, "Z"), 0, 0, 1, 0),
                 "chemical potential": (1, 0, 0, 0, 0, 0, (2.790, '1')),
                 "reaction affinity": ((-1, "A"), 0, 0, 0, 0, 0,
                                       (-2.4790, "AB"))}
    ...
    def build_desired_parameters(self):
        '''
        builds up a dictionary of identifiers for those parameters that are
        chosen to be used for the balancing.
        they are of the kind: [(equilibrium constant, reaction)]=keq_1
        '''
        desired_parameters = {}
        for parameter_type in self.parameter_dict.keys():
            if self.parameter_dict[parameter_type] is True:
                if parameter_type in self.species_parameters:
                    for species in self.species_list:
                        desired_parameters[parameter_type, species] = \
                                self.quantity2identifier[parameter_type] + \
                                '_' + str(self.species2number[species])
                elif parameter_type in self.reaction_parameters:
                    for reaction in self.reaction_list:
                        desired_parameters[parameter_type, reaction] = \
                                self.quantity2identifier[parameter_type] + \
                                '_' + str(self.reaction2number[reaction])
                    ...
                    for reaction_species in self.model_specific:
                        if parameter_type == reaction_species[0]:
                            desired_parameters[parameter_type,
                                               reaction_species[1],
                                               reaction_species[2]] = \
                                                  self.quantity2identifier[parameter_type] + \
                                                  '_' + str(self.reaction2number[reaction_species[1]]) + \
                                                  '_' + str(self.species2number[reaction_species[2]])
    ...
    def collect_available_values(self):
        ...
        # first: get the column indices of the initial SBtab file
        ...
        try: mean_column = self.sbtab.columns_dict['!Mean']
              ...
              self.sbtab.columns_dict['!UnconstrainedGeometricMean']
        try: std_column = self.sbtab.columns_dict['!Std']
        except: std_column = \
              self.sbtab.columns_dict['!UnconstrainedGeometricStd']
        ...
        for row in old_rows:
            ...
            if quantity in self.parameter_dict.keys():
                if len(row) == len(self.sbtab.columns):
                    ...
                    if row_identifier in self.desired_parameters.keys():
                        value_tuples_old.append(single_tuple)
            else:
                raise ParameterBalancingError('There is a quantity type in th'\
                                              'e SBtab that cannot be interpr'\
                                              'eted: %s' % (row[self.sbtab.columns_dict['!QuantityType']]))

        # second: get column indices of the new SBtab file and search it
        for row in self.new_rows:
            if row[self.sbtab_new.columns_dict['!UnconstrainedGeometricMean']] == '':
                continue
            if len(row) == len(self.sbtab_new.columns):
                ...
                if not row[self.sbtab_new.columns_dict['!Reaction:SBML:reaction:id']] == '' and not \
                   row[self.sbtab_new.columns_dict['!Compound:SBML:species:id']] == '':
                    ...
                    row_identifier = (row[self.sbtab_new.columns_dict['!QuantityType']],
                                      row[self.sbtab_new.columns_dict['!Reaction:SBML:reaction:id']])
                else: row_identifier = (row[self.sbtab_new.columns_dict['!QuantityType']],
                                        row[self.sbtab_new.columns_dict['!Compound:SBML:species:id']])

                if row_identifier not in row_identifiers:
                    try:
                        if self.sbtab_new.columns_dict['!Min'] and \
                           self.sbtab_new.columns_dict['!Max']:
                            self.parameter2bounds[row_identifier] = (None, None)
                    except: pass
                if row[self.sbtab_new.columns_dict['!QuantityType']] in self.prior_list and not \
                   self.pseudo_used: continue
                elif row[self.sbtab_new.columns_dict['!QuantityType']] in self.pseudo_list and not \
                     self.pseudo_used: continue
                ...
                if row_identifier not in row_identifiers:
                    ...
                    try:
                        if self.sbtab_new.columns_dict['!Min'] and \
                           row[self.sbtab_new.columns_dict['!Min']] != '':
                            single_tuple.append((float(row[self.sbtab_new.columns_dict['!Min']]),
                                                 float(row[self.sbtab_new.columns_dict['!Max']])))
    ...
    def get_shannons(self):
        '''
        calculate the shannon entropies of the prior and the posterior
        covariance matrices
        '''
        shannons = []
        # first, shannon entropy of prior matrix self.C_prior
        shannon_prior = 0.5 * numpy.log((2 * numpy.pi * numpy.exp(2)) * \
                                        scipy.linalg.det(self.C_prior))
        shannons.append(shannon_prior)

        # second, get shannon entropy of posterior matrix
        shannon_posterior = 0.5 * numpy.log((2 * numpy.pi * numpy.exp(2)) * \
                                            scipy.linalg.det(self.C_post))
    ...
    def make_cpost_string(self):
        '''
        in order to export the posterior covariance matrix to the user, we make
        a tsv-string out of it
        '''
        new_C = '\n'.join(('\t'.join(str(e) for e in row)) for row in self.C_xpost)

        return new_C

    def sample_posterior_dist(self, posterior, posterior_inc, c_matrix_inc,
                              r_matrix, new_SBtab, header, number):
        '''
        give the posterior to this function and receive *number* times a sample
        ...
        for i in range(number):
            ...
            new_posterior = numpy.array(posterior) + \
                            (numpy.dot(numpy.dot(r_matrix, C_root), ksi))

            # last, but not least: get the new SBtab with the sampled values
            new_SBtab = self.build_new_sbtab(new_sbtab=new_SBtab,
                                             posterior_sample=new_posterior,
                                             header=header)
    ...
    def build_dependence_matrix(self):
        '''
        builds the dependence matrix D from all the submatrices needed for the
        balancing
        '''
        # the dependence matrix consists of two major parts: the unit matrix
        # on top and the rows below the unit matrix
        D_matrix = []
        self.parameter2row = {}
        self.quantities = []
    ...
    def build_bottom_row(self, pseudo_quantity):
        ...
        of course, this function is not short. but the build up of the
        specific rows of D is extremely complex and shall be able to be
        generic
        '''
        #######################
        ...
        for i, element in enumerate(use_list):
            ...
            if '!Min' in self.sbtab.columns_dict and \
               '!Max' in self.sbtab.columns_dict:
                self.bounds.append(self.parameter2bounds[pseudo_quantity,
                                                         element])
                ...
                if self.parameter_dict[self.prior_list[j]]:
                    ...
                    try:
                        if len(matrix_type) > 1:
                            ...
                            try:
                                reactants = \
                                            self.reactions_reactants[element][0]
                                stoichiometry_r = \
                                            self.reactions_reactants[element][1]
                                products = \
                                           self.reactions_products[element][0]
                                stoichiometry_p = \
                                            self.reactions_products[element][1]
                            except: pass
    ...
    def build_specific_dependence_matrix(self):
        '''
        the matrix D_x is the dependence matrix that holds all rows from D
        which can be also found in the x_vector. Thus, the dependence matrix
        ...
        for i, single_tuple in enumerate(self.x_vector):
            if single_tuple[0] in self.species_parameters:
                rows.append(self.parameter2row[(single_tuple[0],
                                                single_tuple[2])])
                ...
                rows.append(self.parameter2row[(single_tuple[0],
                                                (single_tuple[1],
                                                 single_tuple[2]))])
            else: print('row identifier not found: ',
                        single_tuple[0], ' ', single_tuple[1],
                        ' ', single_tuple[2])
    ...
    def build_theta_vector(self):
        '''
        generates the theta_vector (default prior means for every parameter
        in model)
        '''
        self.parameter2row = {}
        theta = []
        ...
        for quantity in self.prior_list:
            if self.parameter_dict[quantity]:
                if quantity in self.species_parameters:
                    for species in self.species_list:
                        if (quantity, species) not in used_identifiers:
                            theta.append((quantity,
                                          self.prior_values[quantity][0][0],
                                          species))
                            self.theta_basic.append((quantity,
                                                     self.prior_values[quantity][0][0],
                                                     species))
                            ...
                            if (quantity, species) in self.parameter2bounds.keys():
                                if self.min_column:
                                    self.bounds_inc.append(self.parameter2bounds[(quantity,
                                                                                  species)])
                            used_identifiers.append((quantity, species))
                elif quantity in self.reaction_parameters:
                    for reaction in self.reaction_list:
                        if (quantity, reaction) not in used_identifiers:
                            theta.append((quantity,
                                          self.prior_values[quantity][0][0],
                                          reaction))
                            self.theta_basic.append((quantity,
                    ...
                    for reaction_species in self.model_dict[quantity]:
                        if (quantity,
                            ...
                            if (quantity,
                                (reaction_species[1],
                                 reaction_species[2])) in self.parameter2bounds.keys():
                                if self.min_column:
                                    self.bounds_inc.append(self.parameter2bounds[(quantity,
                                                                                  (reaction_species[1],
                                                                                   reaction_species[2]))])
                            used_identifiers.append((quantity,
                                                     (reaction_species[1],
                                                      reaction_species[2])))
                                ...
                                if (quantity,
                                    ...
                                    if self.min_column:
                                        self.bounds_inc.append(self.parameter2bounds[(quantity,
                                                                                      (reaction_species[1],
                                                                                       reaction_species[2]))])
                                used_identifiers.append((quantity, (reaction_species[1],
                                                                    reaction_species[2])))
                                self.q_prior.append(self.prior_values[quantity][0][0])
    ...
    def calculate_posteriori(self):
        ...
        if self.pseudo_used:
            self.C_post = numpy.linalg.inv(numpy.dot(numpy.dot(self.Q.transpose(),
                                                               self.C_prior_inv),
                                                     self.Q) + \
            ...
            self.C_post = numpy.linalg.inv(self.C_prior_inv + \
                                           numpy.dot(numpy.dot(Q_star_trans,
                                                               self.C_x_inv),
                                                     self.Q_star))
        ...
        if self.pseudo_used:
            self.q_post = numpy.dot(self.C_post,
                                    numpy.dot(numpy.dot(self.Q.transpose(),
                                                        self.C_prior_inv),
                                              self.q_prior) + \
                                    numpy.dot(numpy.dot(Q_star_trans,
                                                        self.C_x_inv),
    ...
    def build_new_sbtab(self, new_sbtab=False, posterior_sample=False,
                        header=False):
        '''
        generates new SBtab
        '''
        ...
        for i, row in enumerate(self.new_rows):
            if len(row) == len(self.new_header):
                ...
                if row[0] in self.species_parameters: row_identifier = (row[0],
                                                                        row[2])
                elif row[0] in self.reaction_parameters:
