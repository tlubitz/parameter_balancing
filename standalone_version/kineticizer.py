...
class kineticizer(object):
    ...
    def _assign_kinetic( self, reaction, params, mode ):
        ...
        if self._enzyme_prefac:
            ...
            '''
            e = misc.get_enzyme_for_reaction(reaction, create=True)
    ...
    def _get_numerator( self, reaction, mode ):
        ...
        if mode=='cat':
            ...
            km_term    = ' * '.join([ get_km_term(s) for s in misc.get_participants(reaction)])
            ...
            hill_coeff = hill_id_temp.replace('$REAC$', reaction.getId())
            #mu_term    = ' + '.join([get_mu_terms(s) for s in misc.get_participants()])
            react_term = ' * '.join([ get_substrate_term(s) for s in reaction.getListOfReactants()]) or '0'
            prod_term  = ' * '.join([ get_substrate_term(s) for s in reaction.getListOfProducts()])  or '0'
            km_term    = ' * '.join([ get_km_term(s) for s in misc.get_participants(reaction)])
            kv   =  self.param2options['kv'][2].replace('$REAC$' ,reaction.getId())
...
class kineticizer_ms(kineticizer):
    ...
    def _get_denominator( self, reaction, mode ):
        get_substr_term   =  lambda s: self._get_substrate_term(s).replace('(', '( 1 + ')
        text = ' * '.join([get_substr_term(s) for s in misc.get_participants(reaction)])
        return '(%s)' %text
