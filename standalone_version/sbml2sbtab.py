...
Python module to convert an SBML file to SBtab file/s.
...
allowed_sbtabs = ['Compartment', 'Compound', 'Reaction',
                  'Rule', 'Quantity', 'Event']
...
class SBMLDocument:
    ...
    def __init__(self, sbml_model, filename):
        ...
        if not self.filename.endswith('.xml') \
           and not filename.endswith('.sbml'):
            raise ConversionError('''The given file format is not
                                     supported: %s''' % (self.filename))
    ...
    def makeSBtabs(self):
        ...
        for sbtab_type in allowed_sbtabs:
            try:
                ...
                if new_sbtab is not False: sbtabs.append(new_sbtab)
            except:
                pass
    ...
    def testForInconvertibles(self):
        '''
        Some SBML entities cannot be converted to SBML. Thus, we add warnings
        to tell the user about omitting them.
        '''
        try:
            rules = self.model.getListOfRules()
            if len(rules) > 0:
                self.warnings.append('''The SBML model contains rules. These
                cannot be translated to the SBtab files yet.''')
        except: pass

    def getRidOfNone(self, sbtabs):
        '''
        Removes empty SBtabs (if no values for an SBtab were provided
        by the SBML).
    ...
    def getRidOfEmptyColumns(self, sbtabs):
        ...
        for i, sbtab in enumerate(sbtabs):
            ...
            for k, element in enumerate(empty_columns):
                empty_columns[k] = str(element)
    ...
    def compartmentSBtab(self):
        ...
        '''
        compartment = [['''!!SBtab SBtabVersion="1.0" Document="%s" TableType="
                        Compartment" TableName="C
                        ompartment"''' % (self.filename.rstrip('.xml'))], ['']]
        ...
        for comp in self.model.getListOfCompartments():
            ...
            if str(comp.getSBOTerm()) != '-1':
                value_row[5] = 'SBO:%.7d' % comp.getSBOTerm()
            try:
    ...
    def compoundSBtab(self):
        ...
        '''
        compound = [['''!!SBtab SBtabVersion="1.0" Document="%s" TableType="Com
                     pound" TableName="
                     Compound"''' % self.filename.rstrip('.xml')], ['']]
        header = ['!Compound', '!Name', '!Location', '!Charge', '!IsConstant',
                  '!SBOTerm', '!InitialConcentration',
    ...
    def eventSBtab(self):
        ...
        event = [['''!!SBtab SBtabVersion="1.0" Document="%s" TableType="Event"
                  TableName="Event"''' % (self.filename.rstrip('.xml'))], ['']]
        header = ['!Event', '!Name', '!Assignments', '!Trigger', '!SBOterm',
            ...
            if eve.getNumEventAssignments() > 1:
                try:
                    ...
                    for ea in eas:
                        ...
                        ea_e = ea.getMath()
                        ea_entry += var + ' = ' \
                                        + libsbml.formulaToL3String(ea_e) + ' | '
    ...
    def ruleSBtab(self):
        ...
        rule = [['''!!SBtab SBtabVersion="1.0" Document="%s" TableType="Rule" T
                 ableName="Rule"''' % self.filename.rstrip('.xml')], ['']]
    ...
    def getAnnotations(self, element):
        ...
        pattern2urn = {"CHEBI:\d+$": "obo.chebi",
                       ...
                       '\.\d+\.(n)?\d+$": "ec-code",
                       "K\d+$": "kegg.orthology",
                       "([A-N,R-Z][0-9]([A-Z][A-Z,0-9][A-Z,0-9][0-9]){1,2})|('\
    ...
    def reactionSBtab(self):
        ...
        '''
        reaction = [['''!!SBtab SBtabVersion="1.0" Document="%s" TableType="Rea
                     ction" TableName=
                     "Reaction"''' % (self.filename.rstrip('.xml'))], ['']]
        header = ['!Reaction', '!Name', '!ReactionFormula', '!Location',
        ...
        for react in self.model.getListOfReactions():
            ...
            if len(modifiers) > 1:
                ...
                for i, modifier in enumerate(modifiers):
                    if i != len(react.getListOfModifiers()) - 1:
            ...
            if str(react.getSBOTerm()) != '-1':
                value_row[6] = 'SBO:%.7d' % react.getSBOTerm()
    ...
    def quantitySBtab(self):
        ...
        quantity_SBtab = '!!SBtab SBtabVersion="1.0" Document="%s" TableType='\
                         '"Quantity" TableName="Parameter"\n!Quantity\t!Param'\
                         'eter:SBML:parameter:id\t!Value\t!Unit\t'\
                         '!Type' % (self.filename.rstrip('.xml'))
        identifiers = []
        the_rows = ''
        ...
        for reaction in self.model.getListOfReactions():
            ...
            if kinetic_law:
                ...
                for parameter in kinetic_law.getListOfParameters():
                    value_row += '%s_%s\t' % (parameter.getId(),
                                              reaction.getId())
                    ...
                    try:
                        ...
                        if "!Identifiers:" not in quantity_SBtab:
    ...
    def makeSumFormula(self, reaction):
        '''
        Generates the reaction formula of a reaction from the list of products
        and list of reactants.
        ...
        for i, reactant in enumerate(reaction.getListOfReactants()):
            if i != len(reaction.getListOfReactants()) - 1:
                if reactant.getStoichiometry() != 1.0:
                    sumformula += str(float(reactant.getStoichiometry())) \
                                  + ' ' + reactant.getSpecies() + ' + '
                else:
        ...
        for i, product in enumerate(reaction.getListOfProducts()):
            if i != len(reaction.getListOfProducts()) - 1:
                if product.getStoichiometry() != 1.0:
                    sumformula += str(float(product.getStoichiometry())) \
                                  + ' ' + product.getSpecies() + ' + '
                else:
...
if __name__ == '__main__':
    ...
    try: sys.argv[1]
        ...
        by also providing an SBML file and an optional SBtab output filename:
        >python sbml2sbtab.py SBMLfile.xml Output''')
        sys.exit()
    ...
    print('''The SBtab file/s have been successfully written to your working
    directory or chosen output path.''')
