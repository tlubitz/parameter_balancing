...
class SBtabDocument:
    ...
    def __init__(self, sbtab, filename=None, tabs=1):
        ...
        If there are more than 1 SBtab file to be converted, provide a "tabs"
        parameter higher than 1.
        ...
        if self.filename.endswith('tsv') or self.filename.endswith('tab') \
           or self.filename.endswith('csv') or self.filename.endswith('.xls'):
            pass
        else: raise ConversionError('''The given file format is not supported:
        %s''' % (self.filename))
    ...
    def checkTabs(self):
        ...
        in a list, moreover stores the SBtab types in a dict linking
        to the SBtabs.
        '''
        self.type2sbtab = {}

        # if there are more than one SBtabs given in single files that might
        # be comprised of several SBtabs:
        if self.tabs > 1:
            for single_document in self.document[0]:
                ...
                for sbtab in sbtabs:
                    ...
                    if sbtabtsv is False: continue
                    new_tablib_obj = tablibIO.importSetNew(sbtabtsv,
                                                           self.filename,
                                                           separator='\t')
                    single_tab = SBtab.SBtabTable(new_tablib_obj,
                                                  self.filename)
        ...
        # elif there is only one document given, possibly consisting of
        # several SBtabs
        else:
            # check for several SBtabs in one document
            ...
            for sbtab in sbtabs:
                as_sbtab = '\n'.join(sbtab)
                new_tablib_obj = tablibIO.importSetNew(as_sbtab,
                                                       self.filename,
                                                       separator='\t')
    ...
    def unifySBtab(self, sbtab):
        '''
        If we have a list of heterogeneous SBtab files, we have to unify them
        to one common delimiter; we choose \t arbitrarily.
        ...
        for row in sbtab:
            ...
            if row.startswith('!'):
                ...
                if '\t' in columns:
                    ...
                    print('''The delimiter of one of the SBtabs could not be
    ...
    def splitDocumentInTables(self, document_rows):
        '''
        If the document contains more than one SBtab, this function splits the
        document into the single SBtabs.
    ...
    def makeSBML(self):
        ...
        while valid:
            ...
            try: self.checkForCompartments()
                ...
                self.warnings.append('''Error: The compartment initialisation
                crashed. Please check for valid compartment information.''')
                break

            # 1st order of bizness: due to the right modeling order of SBML, we
            # first check for a compound SBtab
            if 'Compound' in self.type2sbtab.keys():
                try: self.compoundSBtab()
                except:
                    self.warnings.append('''Warning: The provided compounds
                    could not be initialised. Please check for valid compound
                    information.''')
            else: strikes += 1
            ...
            if 'Reaction' in self.type2sbtab.keys():
                try:
                    ...
                    self.warnings.append('''Error: The provided reaction
                    information could not be converted. Please check for
                    valid reaction information.''')
                    break
            else: strikes += 1
        ...
        else: return False, ['''There was no or not sufficient model information
        available to build an SBML model.''']
    ...
    def checkForCompartments(self):
        '''
        If there is no Compartment SBtab AND no compartments given in the other
        provided SBtab files, a default compartment needs to be set.
        '''
    ...
    def compartmentSBtab(self):
        '''
        Extracts the information from the Compartment SBtab and writes it to
        the model.
        '''
        ...
        for row in sbtab.value_rows:
            if row[sbtab.columns_dict['!Compartment']] not in self.compartment_list:
                ...
                if '!Location:SBML:compartment:id' in sbtab.columns \
                   and row[sbtab.columns_dict['!Location:SBML:compartment:id']] != '':
                ...
                if '!Name' in sbtab.columns \
                   and row[sbtab.columns_dict['!Name']] != '':
            ...
            if '!Size' in sbtab.columns:
                for comp in self.new_model.getListOfCompartments():
                    for compsbtab in sbtab.value_rows:
                        if comp.getId() == compsbtab[sbtab.columns_dict['!Compartment']] \
                           and compsbtab[sbtab.columns_dict['!Size']] != '':
                            comp.setSize(float(compsbtab[sbtab.columns_dict['!Size']]))

            if '!SBOTerm' in sbtab.columns \
               and row[sbtab.columns_dict['!SBOTerm']] != '':
                try:
                ...
                if "Identifiers" in column:
                    ...
                    for pattern in urns:
                        if pattern in column:
                            urn = pattern
                    try:
                        cv_term = self.setAnnotation(compartment, annot, urn,
                                                     'Model')
                        compartment.addCVTerm(cv_term)
                    except:
                        print('''There was an annotation that I could not
                        assign properly: ''', compartment.getId(), annot)

    def compoundSBtab(self):
        '''
        Extracts the information from the Compound SBtab and writes it to the
        model.
        '''
        sbtab = self.type2sbtab['Compound']

        for row in sbtab.value_rows:
            if row[sbtab.columns_dict['!ID']] not in self.species_list:
                species = self.new_model.createSpecies()
                if '!Compound:SBML:species:id' in sbtab.columns \
                   and row[sbtab.columns_dict['!Compound:SBML:species:id']] != '':
                ...
                if '!Name' in sbtab.columns \
                   and not row[sbtab.columns_dict['!Name']] == '':
                    if '|' in row[sbtab.columns_dict['!Name']]:
                        species.setName(str(row[sbtab.columns_dict['!Name']].split('|')[0]))
                ...
                if '!SBML:speciestype:id' in sbtab.columns \
                   and row[sbtab.columns_dict['!SBML:speciestype:id']] != '':
                ...
                if '!Location' in sbtab.columns \
                   and row[sbtab.columns_dict['!Location']] != '':
                ...
                if '!InitialConcentration' in sbtab.columns \
                   and row[sbtab.columns_dict['!InitialConcentration']] != '':
                    species.setInitialConcentration(float(row[sbtab.columns_dict['!InitialConcentration']]))
                elif '!InitialValue' in sbtab.columns \
                     and row[sbtab.columns_dict['!InitialValue']] != '':
                    species.setInitialConcentration(float(row[sbtab.columns_dict['!InitialValue']]))

                if '!IsConstant' in sbtab.columns \
                   and row[sbtab.columns_dict['!IsConstant']] != '':
                ...
                if '!Unit' in sbtab.columns and self.unit_mM is False:
                    ...
                    if "Identifiers" in column:
                        ...
                        for pattern in urns:
                            if pattern in column:
                                urn = pattern
                        try:
                            cv_term = self.setAnnotation(species, annot,
                                                         urn, 'Biological')
                            species.addCVTerm(cv_term)
                        except:
                            print('''There was an annotation that I could not
                            assign properly: ''', species.getId(), annot)

        # species without compartments yield errors --> set them to the first
    ...
    def reactionSBtab(self):
        '''
        Extracts the information from the Reaction SBtab and writes it to
        the model.
        '''
        sbtab = self.type2sbtab['Reaction']

        # if we have the sumformulas, extract the reactants and create the
        # species
        if '!ReactionFormula' in sbtab.columns:
            ...
            for reaction in self.reaction2reactants:
                ...
                for educt in educts:
                    if educt == '': continue
                    if educt not in self.id2sbmlid.keys() \
                       and educt not in self.species_list:
                ...
                for product in products:
                    if product == '': continue
                    if product not in self.id2sbmlid.keys() \
                       and product not in self.species_list:
                        sp = self.new_model.createSpecies()
        ...
        # if compartments are given for the reactions and these compartments
        # are not built yet:
        if '!Location' in sbtab.columns:
            for row in sbtab.value_rows:
                if row[sbtab.columns_dict['!Location']] == '':
        ...
        try:
            ...
            self.warnings.append('''Warning: Please be aware that the SBtab ->
            SBML conversion does not include a validation of the provided
            kinetic rate laws. If they are invalid, this affects the validity
            of the output SBML.''')
        except: pass
        ...
        for row in sbtab.value_rows:
            ...
            if '!SBOTerm' in sbtab.columns \
               and row[sbtab.columns_dict['!SBOTerm']] != '':
                try:
                    react.setSBOTerm(int(row[sbtab.columns_dict['!SBOTerm']][4:]))
                except: pass

            if '!IsReversible' in sbtab.columns \
               and row[sbtab.columns_dict['!IsReversible']] != '':
            ...
            try:
                ...
                if not row[sbtab.columns_dict['!Name']] == '':
                    if '|' in row[sbtab.columns_dict['!Name']]:
                        react.setName(str(row[sbtab.columns_dict['!Name']].split('|')[0]))
                ...
                if row[sbtab.columns_dict['!ReactionFormula']] != '':
                    for educt in self.reaction2reactants[react.getId()][0]:
                        ...
                        if educt in self.id2sbmlid.keys():
                            if self.id2sbmlid[educt] is not None:
                                reactant.setSpecies(self.id2sbmlid[educt])
                            else: reactant.setSpecies(educt)
                        else: reactant.setSpecies(educt)
                        reactant.setStoichiometry(self.rrps2stoichiometry[row[sbtab.columns_dict['!ID']],
                                                                          educt])
                    ...
                    for product in self.reaction2reactants[react.getId()][1]:
                        ...
                        if product in self.id2sbmlid.keys():
                            if self.id2sbmlid[product] is not None:
                    ...
                    if "|" in row[sbtab.columns_dict['!Regulator']]:
                        ...
                        for element in splits:
                            ...
                            if element.startswith('+') \
                               and element[1:] in self.species_list:
                                try:
                                ...
                                try:
                                    ...
                                    mod.setSpecies(element[1:])
                                    self.modifier_list.append(element)
                                    letring = 'Warning: The reaction modifier'\
                                              ' %s could not be identified as'\
                                              'either stimulator or inhibitor'\
                                              '. Please add an SBO'\
                                              'Term.' % (element)
                        ...
                        if (row[sbtab.columns_dict['!Regulator']]).startswith('+') \
                           and row[sbtab.columns_dict['!Regulator']] in self.species_list:
                            try:
                                mod = react.createModifier()
                            ...
                            try:
                                ...
                                self.modifier_list.append(row[sbtab.columns_dict['!Regulator']])
                                letring = 'Warning: The reaction modifier '\
                                          + row[sbtab.columns_dict['!Regulator']]\
                                          + ' could not be identified as '\
                                          'either stimulator or inhibitor'\
                                          '. Please add an SBO Term.'
                                self.warnings.append(letring)
                            except: pass
                        self.modifier_list.append(row[sbtab.columns_dict['!Regulator']])
            except: pass
            ...
            try:
                ...
                if row[sbtab.columns_dict['!MetabolicRegulators']] != '':
                    ...
                    for activator in acts:
                        if activator in self.species_list \
                           and activator not in self.modifier_list:
                    ...
                    for inhibitor in inhs:
                        if inhibitor in self.species_list \
                           and inhibitor not in self.modifier_list:
            ...
            for column in sbtab.columns_dict.keys():
                if "Identifiers" in column:
                    ...
                    print(urn)
                    try:
                        cv_term = self.setAnnotation(react, annot, urn,
                                                     'Biological')
                        react.addCVTerm(cv_term)
                    except:
                        print('There was an annotation that I could not assign
                        properly: ', react.getId(), annot) # , urn

            '''
            # since local parameters need to be entered *after* reaction
            # creation, but *before* setting
            try:
                sbtab.columns_dict['!KineticLaw']
    ...
    def makeUnitDefmpdw(self):
        '''
        Builds unit definition; right now only in case of molecules/gram dry
        weight.
        '''
        ud = self.new_model.createUnitDefinition()
    ...
    def getReactants(self, sbtab):
        ...
        for reaction in sbtab.value_rows:
            if '!ID' in sbtab.columns \
               and reaction[sbtab.columns_dict['!ID']] != '':
            ...
            if sum_formula.startswith('['):
                self.reaction2compartment[r_id] = re.search('[([^"]*)]',
                                                            sum_formula).group(1)
    ...
    def quantitySBtab(self):
        '''
        Extracts the information from the Quantity SBtab and writes it to
        the model.
        '''
                ...
                if row[sbtab.columns_dict['!Description']] == 'local parameter':
                    for reaction in self.new_model.getListOfReactions():
                        ...
                        if row[sbtab.columns_dict['!SBML:reaction:parameter:id']] in formula:
                            ...
                            try:
                                lp.setValue(float(row[sbtab.columns_dict['!Value']]))
                            except: lp.setValue(1.0)
                            try:
                                lp.setUnits(row[sbtab.columns_dict['!Unit']])
                            except: pass
                            if '!Unit' in sbtab.columns \
                               and self.unit_mM is False:
                                if row[sbtab.columns_dict['!Unit']] == 'mM':
                                    self.makeUnitDefmM()
                                    self.unit_mM = True
                                elif row[sbtab.columns_dict['!Unit']].lower().startswith('molecules'):
    ...
    def eventSBtab(self):
        '''
        Extracts the information from the Event SBtab and writes it to
        the model.
        '''
        ...
        for row in sbtab.value_rows:
            ...
            try:
                if row[sbtab.columns_dict['!Trigger']] != '' \
                   and row[sbtab.columns_dict['!Trigger']] != 'None':
            ...
            try:
                if row[sbtab.columns_dict['!Delay']] != '' \
                   and row[sbtab.columns_dict['!Delay']] != 'None':
                ...
                if "Identifiers" in column:
                    ...
                    for pattern in urns:
                        if pattern in column:
                            urn = pattern
                    try:
                        cv_term = self.setAnnotation(event, annot,
                                                     urn, 'Biological')
                        event.addCVTerm(cv_term)
                    except:
                        print('''There was an annotation that I could not
                        assign properly: ''', event.getId(), annot)

    def ruleSBtab(self):
        '''
        Extracts the information from the Rule SBtab and writes it to
        the model.
        '''
                ...
                if "Identifiers" in column:
                    ...
                    for pattern in urns:
                        if pattern in column:
                            urn = pattern
                    try:
                        cv_term = self.setAnnotation(event, annot, urn,
                                                     'Biological')
                        rule.addCVTerm(cv_term)
                    except:
                        print('''There was an annotation that I could not assi
                        gn properly: ''', rule.getId(), annot)
...
if __name__ == '__main__':
    ...
    try: sys.argv[1]
        ...
        by also providing an SBtab file and an optional SBML output filename:
        >python sbtab2sbml.py SBtabfile.csv Output''')
        sys.exit()
    ...
    print('''The SBML file has been successfully written to your working direct
    ory or chosen output path.''')
